---
title: "[源码分析] Flask 配置管理与描述符析"
description: ''
pubDate: "2019-07-03T11:19:00+08:00"
tags: ["Python", "Flask"]
---
...
这一行代码使得 Config 类可以像字典一样使用，比如 `app.config['TESTING']=True`。当然你也可以使用 `__getitem__` 和 `__setitem__` 内置方法使得类具有字典的行为。
...
描述符实现了特定的内置方法，`__get__` ， `__set__` 和 `__delete__` ，常见的比如 Django 中 ORM 中的实现就是用的描述符：
...
`__set__`方法中的 obj 同样是被托管类的实例，然后 value 被存储在被托管类的 config 变量中。所以，我们才可以用 **app.testing = True** 来指定配置。
